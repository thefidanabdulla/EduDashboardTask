...
import { useUpdateHighSchoolMutation } from '../../redux/services/highschool';
import { toast } from 'react-toastify';


export type UpdateHighSchoolData = {
  _id: string;
  name: string;
  address: string;
  principal: string;
...
interface UpdateHighSchoolProps{
  setIsModalShowing: React.Dispatch<React.SetStateAction<boolean>>;
  initialData:UpdateHighSchoolData
}

const UpdateHighSchool = ({ setIsModalShowing, initialData }: UpdateHighSchoolProps) => {
  const [updateHighSchool] = useUpdateHighSchoolMutation();

  const {
    register,
    handleSubmit,
    formState: { errors },
  } = useForm<UpdateHighSchoolData>({
    defaultValues: initialData
  });

  const onSubmit = handleSubmit(async (data) => {
    try {
      await updateHighSchool({...data, _id:initialData._id});
      toast.success("High School updated successfully!!");
      setIsModalShowing(false);
    } catch (error) {
      // console.error('Failed to create school:', error);
      toast.error("Failed to updating high school")
    }
  })
    ...
    <form onSubmit={onSubmit} className='flex flex-col gap-4 py-5'>
        ...
        <label className="text-gray-400 text-sm font-bold flex-1">
          ...
          <input
            type="number"
            className="border rounded-md w-full py-3 px-4 font-normal outline-none mt-2 border-indigo-50"
            {...register("graduationRate", { required: "This field is required" })}
          />
