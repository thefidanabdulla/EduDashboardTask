...
import { useCreateUniversityMutation } from "../../redux/services/university";
import { toast } from 'react-toastify';

export type AddUniversityFormData = {
  _id?: string;
  name: string;
  address: string;
  president: string;
...
const AddUniversityForm = ({ setIsModalShowing }: AddUniversityFormProps) => {
  ...
  const {
    ...
    defaultValues: {
      corpus: [{ department: "", head: "", numberOfCourses: 0 }]
    }
  });
    ...
    <form onSubmit={onSubmit} className='flex flex-col gap-4 py-5'>
      ...
      <div>
        ...
        {fields.map((field, index) => (
          <div key={field.id} className="flex flex-col gap-4 mb-4 border p-4 rounded-md">
            <label className="text-gray-400 text-sm font-bold">
              Department
              <input
                type="text"
                className="border rounded-md w-full py-3 px-4 font-normal outline-none mt-2 border-indigo-50"
                {...register(`corpus.${index}.department`, { required: "This field is required" })}
              />
            ...
            <label className="text-gray-400 text-sm font-bold">
              ...
              <input
                type="text"
                className="border rounded-md w-full py-3 px-4 font-normal outline-none mt-2 border-indigo-50"
                {...register(`corpus.${index}.head`, { required: "This field is required" })}
              />
            ...
            <label className="text-gray-400 text-sm font-bold">
              ...
              <input
                type="number"
                className="border rounded-md w-full py-3 px-4 font-normal outline-none mt-2 border-indigo-50"
                {...register(`corpus.${index}.numberOfCourses`, { required: "This field is required" })}
              />
            ...
            <button type="button" onClick={() => remove(index)} className="mt-2 bg-red-400 text-white rounded-md px-4 py-2">
              Remove
            </button>
          </div>
        ))}
        <button type="button" onClick={() => append({ department: "", head: "", numberOfCourses: 0 })} className="mt-4 bg-blue-400 text-white rounded-md px-4 py-2">
          Add Corpus
        </button>
      </div>
