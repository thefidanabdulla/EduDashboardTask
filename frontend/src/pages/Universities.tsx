...
import { useDeleteUniversityMutation, useGetUniversitiesQuery } from "../redux/services/university";
...
const Universities = () => {
  const [isAddUniversityModalShowing, setIsAddUniversityModalShowing] = useState(true);
  const [isUpdateUniversityModalShowing, setIsUpdateUniversityModalShowing] = useState(false);
  ...
  const { data: universities, isLoading , refetch: refetchUniversities} = useGetUniversitiesQuery();
  ...
  useEffect(() => {
    refetchUniversities();
  }, [isAddUniversityModalShowing, isUpdateUniversityModalShowing, isDeleteModalShowing]);
  ...
  return (
    <div>
      {isAddUniversityModalShowing && (
        ...
        >
          <AddUniversityForm setIsModalShowing={setIsAddUniversityModalShowing} />
        </CustomModal>
      )}
      {isUpdateUniversityModalShowing && (
        ...
        >
          <UpdateUniversityForm initialData={updatedUniversity} setIsModalShowing={setIsUpdateUniversityModalShowing} />
        </CustomModal>
      )}
      ...
      {isCorpusModalShowing && (
        <CustomModal
          title="Corpus"
          setIsModalShowing={setIsCorpusModalShowing}
        >
          <table className="w-full">
            <thead className="w-full">
              <tr className="w-full flex">
                <th className="flex-1  border p-3 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Department</th>
                <th className="flex-1  border p-3 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Head</th>
                <th className="flex-1  border p-3 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Number Of Courses</th>
              </tr>
            </thead>
            <tbody className="w-full">
              {selectedCorpusesData?.map((corpus) => (
                <tr className="w-full flex">
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">{corpus.department}</td>
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">{corpus.head}</td>
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">{corpus.numberOfCourses}</td>
                </tr>
              ))}
            </tbody>
          </table>
            ...
            <table className="w-full">
              <thead className="flex w-full">
                <tr className="flex w-full">
                  ...
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Presitent</td>
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Students count</td>
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Email</td>
                  <td className="flex-1 border p-4 border-indigo-100 text-center text-gray-500 font-semibold text-lg">Corpus</td>
              ...
              <tbody>
                {universities.map((university) => (
                  <tr key={university._id} className="flex w-full">
                    <td className="flex-1 border p-4 border-indigo-100 flex items-center  justify-center text-center text-gray-500 font-medium">{university.name}</td>
                    <td className="flex-1 border p-4 border-indigo-100 flex items-center  justify-center text-center text-gray-500 font-medium">{university.address}</td>
                    <td className="flex-1 border p-4 border-indigo-100 flex items-center  justify-center text-center text-gray-500 font-medium">{university.president}</td>
                    <td className="flex-1 border p-4 border-indigo-100 flex items-center  justify-center text-center text-gray-500 font-medium">{university.students}</td>
                    <td className="flex-1 border p-4 border-indigo-100 flex items-center  justify-center text-center text-gray-500 font-medium">{university.email}</td>
